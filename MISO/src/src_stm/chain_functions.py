...
def find_valid_neighbors_with_intensity(current_point, used_points, points, valid_circles, 
                                       ...
                                       std_threshold=30,pixelsize_angstrom=None):
    """
...
def try_extend_chain_end_enhanced(chain, end_idx, used_points, points, valid_circles, 
                                 ...
                                 intensity_threshold=20, std_threshold=30,pixelsize_angstrom=None):
...
def grow_chain_from_center_enhanced(points, center_idx, valid_circles, original_img,
    ...
    Parameters:
    - points: List of (x, y, z) coordinates in ANGSTROMS
    - center_idx: Index of center point to start from
    - valid_circles: List of [x, y, radius] in ANGSTROMS 
    - original_img: Image for path validation (in pixels)
    - max_bond_distance: Maximum bond distance in ANGSTROMS
    - pixelsize_angstrom: Angstroms per pixel for coordinate conversion
    """
    chain = [center_idx]
    ...
    while len(chain) < len(points):
        if extend_left:
            success, rejected = try_extend_chain_end_enhanced(
                ...
                pixelsize_angstrom  # Pass the conversion factor
            )
        else:
            success, rejected = try_extend_chain_end_enhanced(
...
def generate_chain_networks_enhanced(points, valid_circles, original_img, max_bond_distance, 
                                    pixelsize_angstrom, num_simulations=1000, num_best=10, 
                                    failure_threshold=0.8, intensity_threshold=20, std_threshold=30, seed=None):
    """
    Enhanced chain generation with 2D path validation and intensity-based bonding
    
    Parameters:
    - points: List of (x, y, z) coordinates in ANGSTROMS
    - valid_circles: List of [x, y, radius] in ANGSTROMS for 2D validation
    - original_img: Image for path validation (still in pixels)
    - max_bond_distance: Maximum 3D bond distance in ANGSTROMS
    - pixelsize_angstrom: Angstroms per pixel conversion factor
    ...
    print(f"  Max 3D distance: {max_bond_distance:.1f} Å")
    ...
    for sim in range(num_simulations):
        try:
            chain, unbonded_points, rejected_bonds = grow_chain_from_center_enhanced(
                ...
                intensity_threshold, std_threshold, pixelsize_angstrom
            )
    ...
    for i, chain in enumerate(successful_chains[:num_best]):
        ...
        print(f"  Solution {i}: Length={chain['total_length']:.1f} Å, Bonded={bonded_count}, Unbonded={unbonded_count}")
        if unbonded_count > 0:
